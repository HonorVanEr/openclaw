...
const WHATSAPP_PENDING_HISTORY_CONTEXT = [
  "[Chat messages since your last reply - for context]",
  `Alice (+15550000001): quiet context ${WHATSAPP_PENDING_HISTORY_QUIET_MARKER} ${WHATSAPP_PENDING_HISTORY_CONTEXT_SENTINEL}`,
...
describe("qa mock openai server", () => {
  ...
  it("answers WhatsApp pending-history prompts only with injected prior group context", async () => {
    ...
    const withInjectedContext = await expectResponsesJson(server, {
      stream: false,
      model: "gpt-5.5",
      input: [
        makeUserInput(
          [WHATSAPP_PENDING_HISTORY_CONTEXT, WHATSAPP_PENDING_HISTORY_TRIGGER_PROMPT].join("\n"),
        ),
      ],
    });

    expect(outputText(withInjectedContext)).toBe(WHATSAPP_PENDING_HISTORY_OK_MARKER);
    ...
    const currentMessageOnlySentinel = await expectResponsesJson(server, {
      ...
      input: [
        makeUserInput(
          [
            "[Chat messages since your last reply - for context]",
            "Alice (+15550000001): unrelated prior context",
            "[Current message - respond to this]",
            WHATSAPP_PENDING_HISTORY_TRIGGER_PROMPT,
            `The current trigger text mentions ${WHATSAPP_PENDING_HISTORY_QUIET_MARKER}.`,
            `It also mentions ${WHATSAPP_PENDING_HISTORY_CONTEXT_SENTINEL}.`,
    ...
    const contextWithoutCurrentTrigger = await expectResponsesJson(server, {
      ...
      input: [
        makeUserInput(
          [
            WHATSAPP_PENDING_HISTORY_CONTEXT,
            "openclawqa pending history context check without current trigger",
          ].join("\n"),
        ),
      ],
    });
