...
const QA_WHATSAPP_PENDING_HISTORY_CONTEXT_START =
  "[Chat messages since your last reply - for context]";
const QA_WHATSAPP_PENDING_HISTORY_CURRENT_START = "[Current message - respond to this]";
...
function buildWhatsAppPendingHistoryReply(allInputText: string) {
  ...
  const contextStartIndex = beforeTrigger.lastIndexOf(QA_WHATSAPP_PENDING_HISTORY_CONTEXT_START);
  const currentStartIndex = beforeTrigger.lastIndexOf(QA_WHATSAPP_PENDING_HISTORY_CURRENT_START);
  if (contextStartIndex < 0 || currentStartIndex < 0 || contextStartIndex > currentStartIndex) {
    return "WHATSAPP_QA_PENDING_HISTORY_MISSING_CONTEXT";
  }
  const priorGroupContext = beforeTrigger.slice(contextStartIndex, currentStartIndex);
